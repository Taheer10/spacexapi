...
import { SuccessFullSingleLaunch } from "./SuccessFullSingleLauch";
...
const BodySection = () => {
  ...
  const Api_url = "https://api.spacexdata.com/v3/launches?limit=100&launch_success=true&land_success=true";
    ...
    <div className="container">
      ...
      {posts?.length > 0 ? (
        <div className="contents-section">
          {posts.map((object) => (
            <SuccessFullSingleLaunch key={object.flight_number} object={object}  />
          ))}
        </div>
      ) : (
        <div className="empty">
