...
import { FailedSingleLAnding } from "./FailedSingleLanding";

const BodySection = (year) => {
 
  const [posts, setposts] = useState([]);
  const navigate = useNavigate();

  const Api_url = "https://api.spacexdata.com/v3/launches?limit=100&launch_success=true&land_success=false";
    ...
    <div className="container">
      ...
      {posts?.length > 0 ? (
        <div className="contents-section">
          {posts.map((object) => (
            <FailedSingleLAnding key={object.flight_number} object={object}  />
          ))}
        </div>
      ) : (
        <div className="empty">
